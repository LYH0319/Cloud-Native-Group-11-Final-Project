...
class HeartbeatThread(threading.Thread):
    ...
    def run(self):
        logger.info(f"[Heartbeat] 啟動 Execution ID {self.task.execution_id} 的背景心跳發送...")
        ...
        while not self._stop_event.is_set():
            try:
                ...
                logger.debug(f"[Heartbeat] 已向 Cache 刷新生存訊號: {self.heartbeat_key}")
        ...
        logger.info(f"[Heartbeat] Execution ID {self.task.execution_id} 的心跳監控已安全停止。")


# ==========================================
#          資料庫結果回報同步
# ==========================================
def report_to_database(execution_id: int, status: ExecutionStatus, error_message: str | None = None):
    """
    ...
    """
    logger.info(f"[DB Sync] 正在同步狀態至資料庫 - Execution ID: {execution_id}, 狀態: {status.name}")
    ...
    try:
        ...
        if updated_record and updated_record.duration is not None:
            logger.info(f"[DB Sync 成功] Exec ID {execution_id} 處理結束，系統自動計算耗時: {updated_record.duration} 秒")
...
def process_task(task: TaskPayload, r_client: redis.Redis):
    """處理單個任務的核心生命週期管理"""
    logger.info(f"從佇列中取得任務，準備執行 - Execution ID: {task.execution_id}, Job ID: {task.job_id}")
    ...
    if not r_client.set(idempotency_key, "RUNNING", ex=task.timeout_threshold + 60):
        logger.warning(f"[IDEMPOTENCY] 偵測到重複派發請求！Execution ID {task.execution_id} 正在執行中，自動丟棄此重複任務。")
        return
    ...
    try:
        ...
        report_to_database(execution_id=task.execution_id, status=final_status, error_message=error_msg)
