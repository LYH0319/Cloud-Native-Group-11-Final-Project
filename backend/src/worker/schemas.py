...
class TaskPayload(BaseModel):
    ...
    # 時間限制設定
    timeout_threshold: int = Field(
        default=60, description="長時間任務的超時判定門檻 (秒)"
    )
...
class HeartbeatState(BaseModel):
    ...
    execution_id: int
    status: str = "RUNNING"
    last_active_time: float = Field(
        ..., description="當前的 Unix 時間戳記，供 Monitor 判定是否卡死"
    )
    checkpoint_line: int = Field(
        default=0, description="目前執行到日誌的第幾行 (用於 Shell 任務等)"
    )
    percentage: float = Field(
        default=0.0, description="任務執行進度百分比 (0.0 ~ 100.0)"
    )
