...
def test_users(session):
    ...
    if existing_user:
        ...
        user = User(
            employee_id="104111", username="test_dev_02", role=UserRole.DEVELOPER
        )
        session.add(user)
        session.flush()
        print("✅ 成功新增 User！")
...
def test_jobs_and_infrastructure(session, user):
    ...
    existing_job = session.scalars(
        select(Job).where(Job.owner_id == user.user_id)
    ).first()
    ...
    for job in jobs:
        ...
        if job.executions:
            ...
            for exec_data in job.executions:
                print(
                    f"    - 執行 ID: {exec_data.execution_id} | 狀態: {exec_data.status.name} | 耗時: {exec_data.duration}s"
                )

                # 直接透過 relationship 一路泡茶摸到 Log 資料！
                if exec_data.log_reference:
                    print(
                        f"      - 📝 關聯日誌位置: {exec_data.log_reference.log_path} ({exec_data.log_reference.log_size} Bytes)"
                    )
                else:
                    print("      - 📝 此執行紀錄尚無日誌。")
        else:
