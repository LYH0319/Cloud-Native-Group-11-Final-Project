              ...
              <div style={loginStyles.rightPanel}>
                ...
                {step === 'checkId' && (
                  <form onSubmit={checkIdHandler} style={loginStyles.formContainer}>
                    ...
                    <div style={loginStyles.buttonContainer}>
                      <button type="submit" className="custom-btn" style={loginStyles.primaryBtn} disabled={loading}>
                        {loading ? '檢查中' : '下一步'}
                      </button>
                    </div>
                  </form>
                ...
                {step === 'registerPassword' && (
                  <form onSubmit={registerPasswordHandler} style={loginStyles.formContainer}>
                    <div style={{ ...loginStyles.title, fontSize: '24px' }}>Hi~ 員工{id}！歡迎使用</div>
                    <input 
                      type="password" 
                      style={loginStyles.input}
                      placeholder="輸入您的新密碼"
                    ...
                    <div style={loginStyles.buttonContainer}>
                      <button type="button" className="custom-btn" style={loginStyles.primaryBtn} onClick={backToIdHandler} disabled={loading}>
                        上一步
                      </button>
                      <button type="submit" className="custom-btn" style={loginStyles.primaryBtn} disabled={loading}>
                        {loading ? '處理中...' : '下一步'}
                  ...
                  <div style={loginStyles.successContainer}>
                    <div style={loginStyles.successIcon}>
                      <svg width="64" height="64" viewBox="0 0 64 64" fill="none">
                        ...
                        <path d="M18 32L28 42L46 22" stroke="white" strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    </div>
                ...
                {step === 'resetPassword' && (
                  <form onSubmit={resetPasswordHandler} style={loginStyles.formContainer}>
                    <div style={loginStyles.title}>重新設定員工 <strong>{id}</strong> 的密碼</div>
                    <input 
                      type="password" 
                      style={loginStyles.input}
                      placeholder="輸入您的新密碼"
                    ...
                    <div style={loginStyles.buttonContainer}>
                      <button type="button" className="custom-btn" style={loginStyles.primaryBtn} onClick={() => { setPassword(''); setErrMessage(''); setStep('loginPassword'); setCounter(0);}} disabled={loading}>
                        上一步
                      </button>
                      <button type="submit" className="custom-btn" style={loginStyles.primaryBtn} disabled={loading}>
                        {loading ? '重設中...' : '下一步'}
